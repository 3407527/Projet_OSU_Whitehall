...
int nbPts;
int ptKill[4];
int carresJaunes[6] = {251, 252, 406, 341, 340, 408};
int nbTour;
...
int mouvementAutorise_Police_rec_j(int actuel, int destination, int mov){
  ...
  if ((actuel == destination) && (actuel != ind_v) && (actuel != ind_b)) // si on est arrivé et pas d'autre policier sur la case
	...
	else { // si c'est un carré, on consomme du mouvement
  ...
  while (1)
	...
	case 'B':
	  ...
	  if(fsmServer == 4)
		...
		sendMessageToClient(tcpClients[3].ipAddress, tcpClients[3].port,"E Non valide : Veuillez selectionner l'un des carres jaunes.");
	    }
	  ...
	  if (fsmServer == 9){
		...
		if ((liaisons[ind_b][i] <= 189) && (liaisons[ind_b][i] >= 1)){
	      ...
	      broadcastMessage("T Le policier Jaune choisit une action a effectuer.");
	      fsmServer = 10;
	    }
	    else
	  ...
	  if (fsmServer == 18){
		...
		if (est_present(ptKill, ind_k, 4)){ // si meurtre
		  ...
		  if (nbTour == 15){ // si dernier tour
		    broadcastMessage("T Jack n'a pas élimine une cible a temps! Les policiers ont gagne!");
		    fsmServer = 19;
		  }
		  else { // sinon on continue normalement
		    nbTour++;
		...
		sprintf(reply, "T Jack a ete arrete par le policier Bleu en %d! Fin de la partie !", indice);
		broadcastMessage(reply);
		fsmServer = 19;
	      }
	    }
